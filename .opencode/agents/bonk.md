...
protocol specification (`packages/docs/src/content/docs/reference/protocol.mdx`).
...
**Wire protocol:** The protocol is specified in `packages/docs/src/content/docs/reference/protocol.mdx`. Serialization changes must remain compatible with existing peers; intentional protocol changes must update that document in the same PR. More broadly, `packages/docs/` is the source of truth for all user-facing documentation; behaviour changes should update the relevant page there, not the README.
